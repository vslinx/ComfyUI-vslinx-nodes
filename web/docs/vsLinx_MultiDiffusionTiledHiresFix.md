...
- <b>Upscale VAEs are supported.</b> If the connected VAE is an upscale VAE from a pack like <a href="https://github.com/spacepxl/ComfyUI-VAE-Utils">ComfyUI-VAE-Utils</a> (e.g. the Wan2.1 <code>upscale2x</code> image VAE), its decoder outputs extra channels (<code>3 × k²</code>) that must be unshuffled into a <code>k</code>-times larger RGB image. The node auto-detects this and decodes the same way that pack's own decode node does, so you get a correct (and larger) image instead of a broken multi-channel result. Normal VAEs are unaffected. This detection also applies when ``vae_decode_tiled`` is enabled.
