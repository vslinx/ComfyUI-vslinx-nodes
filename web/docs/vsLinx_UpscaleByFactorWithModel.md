...
| upscale_method | ``nearest-exact`` / ``bilinear`` / ``area`` / ``lanczos`` | The resampling method used for the final resize step to match your target factor. |
...
- ``area`` generally works best for downscaling; ``nearest-exact`` preserves hard edges but can look blocky; ``bilinear`` is smoother but may soften details; ``lanczos`` gives the sharpest, highest-quality resize but runs on the CPU (via PIL), so it is somewhat slower than the other methods.
